...
- (BOOL)isEqual:(HTMLPurifier_AttrDef_HTML_LinkTypes*)other
{
    if (other == self)
        return YES;
    ...
    if(![other isKindOfClass:[HTMLPurifier_AttrDef_HTML_LinkTypes class]])
        return NO;
    
    BOOL selectorEqual = (!self.name && ![other name]) || [self.name isEqual:[other name]];
