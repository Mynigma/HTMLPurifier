...
{
    return [self registerModule:module withConfig:config overload:NO];
}
...
*/
    ...
    {
        [self processModule:module withConfig:config];
        [self.registeredModules[module] setup:config];
    }
    ...
    self.contentSets = [[HTMLPurifier_ContentSets alloc] initWithModules:self.modules];
...
- (void)processModule:(NSString*)module withConfig:(HTMLPurifier_Config *)config
{
    ...
    {
        moduleName = [self registerModule:module withConfig:config];
    }
    if(moduleName && self.registeredModules[moduleName])
        [self.modules addObject:self.registeredModules[moduleName]];
}
...
- (HTMLPurifier_ElementDef*)getElement:(NSString*)name trusted:(BOOL)trusted
{
    ...
    {
        HTMLPurifier_HTMLModule* module = nil;
        ...
        if(!module)
        {
            TRIGGER_ERROR(@"Module with name %@ not found", module_name);
            continue;
        }
